...
def agregarTarea(
    tareas: dict, nombre: str, dificultad: str, estado: str, vencimiento: str
):
...
def mostrarTareas(tareas: dict):
    if tareas:
        ...
        for descripcion, detalles in tareas.items():
            print(
                f"🔹 {descripcion}: {detalles["estado"]} - |{detalles["vencimiento"]}| - nivel: 🔥 {detalles["dificultad"]} "
            )
    else:
        print("No hay ninguna tarea aun ❗❕ \n")
